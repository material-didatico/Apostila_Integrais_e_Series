...
\begin{itemize}
  \item Quando tentamos resolver a integral $\int x \sqrt{a^2-x^2}dx$,
        podemos usar uma substituição simples $u=a^2-x^2$.

  \item No geral fazemos a seguinte mudança
        \[
          \int f(g(x))g'(x) dx= \int f(u)du
        \]

  \item Para a integral $\int \sqrt{a^2-x^2}dx$ não conseguimos fazer a
        substituição simples

  \item Nesse caso fazemos uma substituição ``inversa''
  $x=a\sin( \theta )$, assim:
  \begin{align*}
    \int \sqrt{a^2-x^2}dx
    & = \int \sqrt{a^2-a^2 \sin^2(\theta)} a\cos(\theta) d \theta \\
    & = \int \sqrt{a^2(1-\sin^2(\theta))} \cos(\theta) d\theta \\
    & = \int a^2 \cos^2 (\theta) d\theta
  \end{align*}

  \item Nesse caso geral temos
        \[
          \int f(x)dx= \int f(g(\theta)) g'(\theta) d \theta
        \]

  \item Podemos fazer a substituição inversa desde que $g$ seja injetiva.

  \item No nosso caso  podemos fazer a substituição inversa $x=a\sin(\theta)$,
        desde que essa função seja injetora. Isso pode ser conseguido se
        restrigirmos $\theta$ ao intervalo
        \(\left[-\frac{\pi}{2}, \frac{\pi}{2}\right]\)

\end{itemize}
Nessa seção estudaremos a técnica de substituição trigonométrica e ela serve
para integrar funções que tem uma ''raíz'' e não conseguimos usar nenhuma das
...
\begin{table}
  ...
  \caption{Substituições Trigonométircas}
  \label{tab:substituicoes_trigonometricas}
  \begin{tabular}{cp{7cm}c}
    \hline
       Expressão     & Substituição                                                       &             Identidade             \\ \hline
    ...
    $\sqrt{x^2-a^2}$ & $x=a\sec(\theta)$, $0 \leq \theta < \frac{\pi}{2}$                 &  $\sec^2(\theta)-1=\tan^2(\theta)$ \\
                     & ou $\pi \leq \theta < \frac{3\pi}{2}$                              &                                    \\ \hline
  \end{tabular}
\end{table}
%------------------------------------------------------------------------------%
...
\begin{example}
  ...
  $x=3\sin(\theta) \Rightarrow \sin(\theta)= \frac{x}{3}$. Se interpretarmos
  ...
  $\sin(\theta)=\frac{x}{3}$.
  O Teorema de Pitágoras nos dá que o cateto adjascente é
  $\sqrt{9-x^2}$. Assim,
  \[
  ...
  então $\theta = \arcsin(\frac{x}{3})$, logo
  \[
    \int \frac{\sqrt{9-x^2}}{x^2}dx
    = -\cot(\theta)-\theta+C
...
\end{example}
%------------------------------------------------------------------------------%
Durante a resolução do exercício usamos que $\sqrt{\cos^2(\theta)}=
\cos(\theta)$, fizemos isso porque $\theta \in [-\frac{\pi}{2}, \frac{\pi}{2}]$
e $\cos(x)$ é não-negativo nesse intervalo, assim, $|\cos(x)|= \cos(x)$, nesse
caso.
...
Durante a resolução do exercício usamos o fato de que
...
\begin{example}
  ...
  substituições trigonométricas. No entanto, temos uma raíz no denominador.
  Vamos manipular a expressão dentro da raíz para chegarmos em uma das
