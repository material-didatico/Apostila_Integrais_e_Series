...
\emph{Quando usar?} Normalmente, usamos integração por partes quando queremos
...
\(
  \int x \sin(x) dx
\),
temos um produto de funções onde $\sin(x)$, por
exemplo, é a derivada de alguém que conheço, isto é, de $-\cos(x)$.

\(
  \int x \ln(x) dx
\),
aqui também temos um produto de funções e $x$ é a
derivada de \(\frac{x^2}{2}\).

\(
  \int (x^2+1) e^{-x} dx
\),
aqui temos um produto de duas funções e
$e^{-x}$ é a derivada de $-e^{-x}$.

Nossa intenção ao usarmos integração por partes é obtermos uma integral
...
É útil chamar $u=f(x)$ e $dv=g'(x)dx$ e usar o padrão
\begin{align*}
  u  & = f(x)    &  dv & = g'(x)dx \\
  du & = f'(x)dx &  v  & = g(x)
\end{align*}
...
Na fórmula de integração por partes, $g$ é qualquer
antiderivada de $g'$, então escolhemos a constante igual a zero por simplicidade.

Você não deve escolher $u$ e $dv$ de qualquer maneira.
...
\begin{example} \label{ex:ex1partes}
  ...
  Assim, usando a fórmula \ref{eq:formula_partes-1}, obtemos
  \begin{align*}
    \int x\sin(x)dx
    & = f(x)g(x)- \int g(x)f'(x)dx \\
    & = x\big(-\cos(x)\big)- \int -\cos(x)dx \\
...
\begin{example}
  ...
  \[
    ...
    = t e^t - e^t + C
  \]
  Substituindo esse resultado na equação \eqref{eq:exemplo_r}, obtemos
  \begin{align*}
    \int t^2 e^t dt
...
\begin{example}
  ...
  Porém, se substituirmos a expressão
  \(
    \int e^x \cos(x)dx
  \) da
  ...
  \[
    \int e^x \sin(x) dx = \frac{e^x}{2}\big( -\cos(x) +\sin(x) \big) + C
  \]
\end{example}
%------------------------------------------------------------------------------%
...
\item
  ...
  \begin{align*}
    A
    & = \left[ -x \cos(x) + \sin(x) \right] \evalat{0}{\pi} \\[2mm]
    & = \big(-\pi\cos(\pi+ \sin(\pi))\big)
  ...
  \begin{align*}
    \int -2x \cos(x) \, dx
    & = -2x \sin(x) - \int -2 \sin(x) \, dx \\[2mm]
    & = -2x \sin(x) - 2 \cos(x)+K
  \end{align*}
  Agora, voltamos à primeira expressão
  \begin{align*}
    \int x^2 \sin(x) \, dx
    & = -x^2 \cos(x) - (-2x \sin(x) - 2 \cos(x))+K \\[2mm]
    & = -x^2 \cos(x) + 2x \sin(x) + 2 \cos(x)+K
  \end{align*}
  ...
  \begin{align*}
    V & = 2\pi \int_{0}^{\pi} x^2 \sin(x) \, dx                                     \\
      ...
      & \hspace{14mm} - \big(0^2 \cos(0) + 2(0) \sin(0) + 2 \cos(0)\big) \Big]
  \end{align*}
