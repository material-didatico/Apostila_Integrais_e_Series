...
%------------------------------------------------------------------------------%
\newcommand{\mytitle}   {Séries Numéricas}
\newcommand{\mysubtitle}{Definição e Exemplos}
\title   {\mytitle}
\subtitle{\mysubtitle}
...
\begin{frame}{Sequência de Somas Parciais}
    ...
    \pause
    Temos agora uma nova sequência de \emph{Somas Parciais} $(S_n)$
...
\begin{frame}{Demonstração dor Indução Finita}
	...
	\begin{enumerate}[<+->]
		...
		\item se ela for verdadeira pra $n$ então também será para $n+1$
	\end{enumerate}
\end{frame}
...
\begin{frame}{Demonstração da Fórmula das Somas Parciais}
    ...
    \(\quad S_{n-1} = 2 - \frac{1}{2^{n-2}} \) \pause
    \begin{align*}
    	\action<+->{S_n & = 1 + \frac{1}{2}
                            ...
                            + \frac{1}{2^{n-2}}
                            + \frac{1}{2^{n-1}}} \\[2mm]
    	\action<+->{& = S_{n-1} + \frac{1}{2^{n-1}}\\[2mm]}
    	\action<+->{& = \textcolor{blue}{2 - \frac{1}{2^{n-2}}} + \frac{1}{2^{n-1}}\\[2mm]}
    	\action<+->{& = 2 - \frac{2}{2^{n-1}} + \frac{1}{2^{n-1}}\\[2mm]}
...
\begin{frame}{Séries Geométricas}
  ...
  \]
	onde
	\begin{description}
		\item[$\alpha$] $\in\R\quad$ número fixo não nulo, $\alpha\neq 0$
		\item[$r$]      $\in\R\quad$ é a \emph{razão} da série
	\end{description}
...
\begin{frame}{Somas Parciais}
	\begin{align*}
		...
		\onslide<+->{& = \left( \frac{1}{1} - \frac{1}{1+1} \right)
			...
			\\[3mm]}
		\onslide<+->{& = \left( \frac{1}{1} - \cancel{\frac{1}{2}} \right)
