...
\begin{example}
  ...
  e a potência de $\sin(x)$ é ímpar,
  por isso, guardamos um fator $\sin(x)$ e usamos a transformação
  $\sin^2(x)= 1- \cos^2(x)$,
  decorrente da relação fundamental. Assim,
  \[
  ...
  \[
    ...
    = \int \left(1-\cos^2\right)cos^2(x)\sin(x) dx
  \]
...
$\sin^2(x)=1-\cos^2(x)$?
Bom, caso fizéssemos essa mudança, teríamos a seguinte integral resultante
\[
  \int \sin^3(x)\cos^2(x) dx
  = \int \sin^3(x)\big(1-\sin^2(x) \big) dx
  = \int \big[ \sin^3(x)- \sin^5(x) \big] dx
\]
...
\begin{example}
  ...
  usamos a substituição simples
  \[
    u = \sin(x) dx \Leftrightarrow du=\cos(x)dx.
  \]
  Com isso,
  \begin{align*}
    \int \cos^3(x)dx & = \int \cos^2(x) \cos(x)dx            \\
                     & = \int \big(1-\sin^2(x)\big)\cos(x)dx \\
...
\begin{example}
  ...
  \begin{align*}
    \int_0^{\pi} \sin^2(x) dx
    & = \int_0^\pi \frac{1}{2}\big(1-\cos(2x)\big) dx \\
    & = \frac{1}{2} \int_0^\pi \big(1-\cos(2x)\big) dx \\
...
\begin{example}
  ...
  \begin{align*}
    ...
    & = \int \frac{1}{2}\big(1- \cos(2x)\big) \frac{1}{2}\big(1+ \cos(2x)\big) dx \\
    & = \frac{1}{4} \int \big(1- \cos^2(2x)\big) dx
  \end{align*}
  Precisamos mais uma vez usar a fórmula do arco metade escrevendo
  $\cos^2(2x)= \frac{1}{2}\big(1+ \cos(4x)\big)$, portanto:
  \begin{align*}
    \int \sin^2(x) \cos^2(x) dx
    & = \frac{1}{4} \int \big(1- \cos^2(2x)\big) dx \\
    & = \frac{1}{4} \int 1-\frac{1}{2}\big(1+ \cos(4x)\big)dx \\
...
\begin{example}
  ...
  \begin{align*}
    ...
    & = \int \tan^6(x) \big(1+ \tan^2(x)\big)  \sec^2(x)dx \\
...
\begin{example}
  ...
  o fator $\sec(x)\tan(x)$ e usamos a transformação
  $\tan^2(x)= \sec^2(x)-1$.
  ...
  \begin{align*}
    ...
    & = \int \big(\sec^2(x)-1\big)^2\sec^6(x) \sec(x) \tan(x) dx \\
    & = \int \left(u^2-1\right)^2 u^6 du \\
